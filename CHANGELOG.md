...
- Clear stale agent error indicators after saving Settings and reload failed
...
- Make normal messages steer active work by cancelling the current turn and
  dispatching to its snapshotted recipient. Add `/queue MESSAGE` for explicit
  non-interrupting FIFO delivery.
- Render fenced code as a clean background block without decorative box
  characters, while preserving indentation and scrolling behavior.
...
- Shorten the generic `Run command` tool label to `bash` in conversation
  previews while preserving descriptive provider titles.
...
- Surface ACP output-token exhaustion as a failed turn instead of silently
  completing, and keep no-output failures and usage-limit errors visible in
...
- Removed generic tool-call labels and completion counts from tool summaries.
...
- Resuming ACP history no longer starts agent timers or blocks new prompts.
  Replayed messages and tools are display-only; work starts on a new human prompt.
...
- Removed the transcript scrollbar and its settings toggle; scrolling and
  follow-output continue to use the full conversation width.
