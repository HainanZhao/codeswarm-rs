...
| Persistent prompt/settings preferences | Implemented | Roster, custom agents, project-scoped prompt history, prompt placeholder, follow-tail, collapsed-details, notification policy, thoughts, tool-expansion, density, diff view, title-blink, and Terminal/Light/Dark theme preferences persist. Terminal focus remains transient runtime state. |
...
| Session history browser | Implemented | `/sessions` browses multiple local archives by stable ID; `/resume` opens history without providers, and a new prompt explicitly reconnects. `/status` and `/summary` expose diagnostics and recorded outcomes. |
