...
| Live roster reconfiguration | Implemented (bounded) | Rust supports a catalog-backed `/config` editor with idle-session add/drop/promote/swap reconciliation (including starting and promoting a newly selected owner), next-launch persistence, and live `/add`, `/drop SLOT`, `/promote SLOT`, `/swap A B`; runtime roster/owner/session snapshots persist Python-compatible owner fields off-thread for both relay and standalone launches, with resumability gated by adapter capability. |
| Persistent prompt/settings preferences | Implemented | Roster, custom agents, Python-compatible project-scoped prompt history, prompt placeholder, follow-tail, collapsed-details, notification policy, thoughts, tool-expansion, density, scrollbar, diff view, and title-blink preference persist. Theme is a fixed product palette and terminal focus is intentionally transient runtime state, not a user preference. |
